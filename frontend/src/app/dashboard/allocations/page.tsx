...
import { ArrowRight, History, Check, X } from "lucide-react";
...
export default function AllocationsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [ass, emps, alls, trans] = await Promise.all([
        apiFetch("/assets"),
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isAllocated) {
        await apiFetch("/transfers", { method: "POST", body: JSON.stringify({ assetId: selectedAssetId, targetUserId: targetUserId, reason: transferReason }) });
  ...
  const handleReject = async (id: string) => {
    try {
      await apiFetch(`/transfers/${id}/reject`, { method: "POST", body: JSON.stringify({ reason: "Rejected" }) });
      fetchData();
    } catch(e) { console.error(e); }
  };
      ...
      {initialLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-2 space-y-6">
            <div className="rounded-xl glass-card p-6">
              <div className="mb-6">
                ...
                <select
                  ...
                  className="w-full px-4 py-2.5 rounded-xl border text-[14px] font-medium outline-none transition-all focus:ring-2 focus:ring-[var(--accent)]/30 focus:border-[var(--accent)] appearance-none cursor-pointer"
              ...
              {selectedAsset && (
                <div className="border-t pt-5 border-[rgba(255,255,255,0.05)]">
                  ...
                  <div className="mb-5">
                    ...
                    <textarea
                      rows={3}
                      className="w-full px-4 py-3 rounded-xl border text-[14px] font-medium outline-none resize-none transition-all focus:ring-2 focus:ring-[var(--accent)]/30 focus:border-[var(--accent)] placeholder-slate-400"
                      style={{ background: "var(--bg-elevated)", borderColor: "var(--border)", color: "var(--text-primary)" }}
            ...
            {(isAdmin || isManager) && pendingTransfers.length > 0 && (
              <div className="rounded-xl glass-card p-6">
                <h3 className="text-[14px] font-semibold mb-4 tracking-wide" style={{ color: "var(--text-primary)" }}>Pending Transfer Requests</h3>
                <div className="space-y-3">
                  {pendingTransfers.map(t => (
                    <div key={t.id} className="p-4 rounded-xl border border-[rgba(255,255,255,0.05)] bg-[rgba(0,0,0,0.2)] flex justify-between items-center">
                      <div>
                        <p className="text-[13px] font-medium" style={{ color: "var(--text-primary)" }}>{t.asset?.name} ({t.asset?.assetTag})</p>
                        <p className="text-[12px] text-slate-400 mt-1">Requested by {t.requestedBy?.firstName} for {t.targetUser?.firstName}</p>
                        <p className="text-[12px] text-amber-500/80 mt-1">Reason: {t.reason}</p>
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => handleApprove(t.id)} className="p-2 rounded-lg bg-green-500/10 text-green-400 hover:bg-green-500/20 transition-colors" title="Approve">
                          <Check size={16} />
                        </button>
                        <button onClick={() => handleReject(t.id)} className="p-2 rounded-lg bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-colors" title="Reject">
                          <X size={16} />
                        </button>
                      </div>
                    </div>
